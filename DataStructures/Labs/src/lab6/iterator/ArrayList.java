	...
	would return an element rather than throwing an exception.)
  	...
  	public E next() throws NoSuchElementException {
  		if(hasNext()) {
  			...
  			throw new NoSuchElementException("out of the index array");
  		}
  	}
  	/*
  	 Returns the previous element in the list and moves the cursor position
		backwards. It throws NoSuchElementException if the iteration has no previous element. This method may be called repeatedly to iterate through the list backwards, or
...
//------------ end of nested MyListIterator class ------------
  ...
  public MyIterator<E> MyListIterator(int j) {
  ...
  public static void removepositions(ArrayList<Integer> L,ArrayList<Integer> P) {
	  MyIterator pig=L.MyListIterator();
	  MyIterator butcher=P.MyListIterator();
	  int cutlet=0;
	  butcher.next();
	  pig.next();
	  while(pig.hasNext()) {
		  
		  if(butcher.current()!=null) {
			  if(cutlet==(int)butcher.current()) {
				  
				  System.out.println("Cut me out index"+cutlet);
				  System.out.println("butcher value"+butcher.current());
				  System.out.println(" ");
				  cutlet++;
				  pig.remove();
				  pig.next();
